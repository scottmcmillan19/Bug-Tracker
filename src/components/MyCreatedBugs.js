...
class MyCreatedBugs extends React.Component {
    state = { bugs: null };

    componentDidMount() {
        // get one's created bugs and sort by status
        let token = localStorage.getItem("auth-token");
        axios.get('/api/myCreatedBugs', { headers: { "x-auth-token": token } })
            .then(res => {
                res.data.sort((a, b) =>
            ...
            return (
                ...
                bugs are in progress bugs.<a style={{color: "red", display: "inline"}}> Red</a> bugs are finished bugs.  </p>
                    ...
                    <ul className="bugKinds">
                        <li><Link to="/bugs/myCreatedBugs" style={{backgroundColor: "white"}}>My Created Bugs</Link></li>
